...
class SaleOrder(models.Model):
    ...
    def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0, **kwargs):
        ...
        if line_id:
            ...
            if line_obj and line_obj.booking_slot_id:
                flag = 1
                booking_price = line_obj.price_unit
        ...
        if res.get('line_id') and flag==1 and res.get('quantity')!=0:
            ...
            line_obj.price_unit = booking_price
...
class SaleOrderLine(models.Model):
    ...
    booking_slot_id = fields.Many2many("booking.slot", 'booking_order_rel', 'book_id', 'order_id', string="Booking Slot")
    booking_date = fields.Date(string="Activity Date")
    ...
    def _onchange_categ(self):
        return {'domain': {'product_id': [('categ_id', '=', self.product_categ_id.id), ('hydro_location_ids', 'in', self.hotel_id.id)]}}
        ...
        for rec in self:
            ...
            if product_obj and product_obj.is_booking_type:
                ...
                if rec.booking_slot_id:
                    for book in rec.booking_slot_id:
                        name += " (" + book.name_get()[0][1] + ")"
                    rec.name = name
        return
    ...
    def is_booking_date_valid(self):
        for rec in self:
            if rec.booking_date:
                ...
                if rec.product_id:
                    sol_obj = self.env["sale.order.line"].search(
                        [("booking_date", "=", rec.booking_date), ("product_id", "=", rec.product_id.id)])
                    lines = []
                    for book in sol_obj.booking_slot_id:
                        avl_qty = rec.product_id.product_tmpl_id.get_bk_slot_available_qty(rec.booking_date, book._origin.id)
                        if avl_qty < 1:
                            lines.append(book.id)
            ...
            #                 raise UserError(_('Available quantity on %s for %s(%s) is %s' % (rec.booking_date, book.time_slot_id.name_get()[0][1], book.plan_id.name, avl_qty)))
                domain = [('slot_config_id','=',day_valid.id ), ('id','not in',lines )]
                return {
    ...
    def onchange_quantity_available(self):
        for rec in self :
            ...
            if rec.booking_slot_id:
                rec.price_unit = 0
                for line in rec.booking_slot_id:
                    rec.price_unit += line.price

    def _get_display_price(self, product):
        for rec in self:
            if rec.booking_slot_id:
                for book in rec.booking_slot_id:
                    currency_id = book.with_context(pricelist=self.order_id.pricelist_id.id).currency_id
                    price = book.with_context(pricelist=self.order_id.pricelist_id.id).price
                    converted_price = currency_id._convert(price,self.order_id.pricelist_id.currency_id,self.company_id,fields.Date.today())
...
class ProductTemplate(models.Model):
    ...
    def get_bk_slot_available_qty(self, bk_date, slot_plan_id):
        ...
        sol_objs = slot_plan_obj.sudo().line_ids.filtered(lambda line: line.booking_date == bk_date).mapped('product_uom_qty')
    ...
    def get_booking_slot_for_day(self, sel_date):
        ...
        for slot in time_slots:
            ...
            for d in d_slots:
                avl_qty = self.get_bk_slot_available_qty(sel_date, d)
                if avl_qty:
                    d2.append({
                        'name' : d.plan_id.name,
                        'id' : d.id,
