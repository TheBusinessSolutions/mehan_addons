...
class WebsiteSale(WebsiteSale):
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        if bk_plan:
            ...
            currency_id = bk_slot_obj.currency_id
            sale_order = request.website.sale_get_order()
            order_line = sale_order.order_line.filtered(lambda l: l.product_id.id == int(product_id))
            converted_price = currency_id._convert(bk_slot_obj.price,order_line.currency_id,order_line.company_id,fields.Date.today())
            order_line.write({
...
class BookingReservation(http.Controller):
    ...
    def get_default_slot_and_plans(self, current_day_slots):
        for rec in current_day_slots:
            if rec.get('plans'):
                default_slot = rec['slot']['id']
                return default_slot, rec['plans']
        return None, None
    ...
    def get_all_week_days(self, sel_date, product_obj):
        ...
        week_start = request.env['res.lang'].search([('code','=',request.context.get('lang'))],limit=1)
        if week_start:
            week_start = week_start.week_start
            ws_index = int(week_start) -1
            week_days_as_lang = list_of_days[ws_index:]
            week_days_as_lang.extend(list_of_days[:ws_index])
            day = Days[sel_date.weekday()]
            v_date = sel_date - timedelta(days=week_days_as_lang.index(day))
        else:
    ...
    def booking_reservation_update_slots(self,**post):
        ...
        default_slot, default_plans = self.get_default_slot_and_plans(current_day_slots)
        values = {
