...
#     price_subtotal = fields.Float(string='Subtotal', readonly=True, compute = '_price_subtotal',)
    ...
    product_serial_ids = fields.Many2many('custom.serial.number', 'product_serial_rel', 'pro', 'ser')
    ...
    @api.constrains('product_serial_ids')
    def _check_exist_serial_in_line(self):
        for rec in self:
            exist_product_list = self.env['analytic.sale.order.line'].search(
                [('subscription_product_line_id', '=', rec.subscription_product_line_id.id), ('product_serial_ids', 'in', rec.product_serial_ids.ids)])
            if len(exist_product_list) > 1:
                raise ValidationError(_('Serial should be one per line.'))
