odoo.define("product_multi_uom_pos.models", function (require) {
  ...
  models.Orderline = models.Orderline.extend({
    get_po_uom: async function (orderline) {
      return rpc.query({
        model: "product.template",
        method: "get_po_uom",
        args: [orderline.product.product_tmpl_id],
      });
    },

    set_init_lst_price: function (orderline) {
      ...
      if (lst_uom.uom_type == "bigger") {
        ...
        var latest_price = ref_price / lst_uom.factor;
        orderline.set_unit_price(latest_price);
        orderline.lst_price = latest_price;
      } else {
        orderline.set_unit_price(ref_price);
        orderline.lst_price = latest_price;
      }
    },
    /*Adding uom_id to orderline*/
    initialize: async function (attr, options) {
      OrderlineSuper.prototype.initialize.call(this, attr, options);

      var orderline = this;

      var po_uom = await orderline.get_po_uom(orderline);
      orderline.uom_id = orderline ? po_uom : [];

      var order = orderline.order;
      orderline.set_init_lst_price(orderline);
      console.log("debug16");
