odoo.define("product_multi_uom_pos.models", function (require) {
  ...
  models.Orderline = models.Orderline.extend({
    ...
    //     OrderlineSuper.prototype.get_product.call(this);
    ...
    initialize: function (attr, options) {
      ...
      this.uom_id = this ? [36, "شريط flumox"] : [];
      this.product.org_lst_price = this.product.lst_price;
      this.set_lst_price(10);
      //   this.set_quantity2(1, this);
      //   this.order.add_orderline(this);
      rpc
        .query({
          model: "pos.order",
          method: "get_product_template",
          args: [this.product.product_tmpl_id],
        })
    ...
    //   console.log(
    //     "SSSSSSSSddddddddSSSSSSSSSS",
    //     this.pos.get_order().get_orderlines()
    //   );
    //   console.log(
    //     "SSSSSSSSPPPPPPPPPPPPSSSSSSSSSS",
    ...
    set_quantity: function (quantity, keep_price) {
      ...
      if (!keep_price && !this.price_manually_set) {
        ...
        if (orderline) {
          ...
          var ref_price = orderline.product.org_lst_price;
    ...
    // set_quantity2: function (quantity, orderline) {
    //   var uom = orderline.uom_id[0];
    //   var lst_uom = this.pos.units_by_id[uom];
    //   var ref_price = orderline.product.lst_price;

    //   if (lst_uom.uom_type == "bigger") {
    //     var latest_price = ref_price * lst_uom.factor_inv;
    //     console.log("biggerrrrrrrrrrrrr", latest_price)

    //     // orderline.order.remove_orderline(orderline);
    //     orderline.order.add_orderline(orderline);

    //     this.set_unit_price(latest_price);
    //     this.order.fix_tax_included_price(this);
    //     orderline.lst_price = latest_price;

    //   } else if (lst_uom.uom_type == "smaller") {
    //     console.log("smallerrrrrrrrrrrrr", ref_price / lst_uom.factor)
    //     this.set_unit_price(ref_price / lst_uom.factor);
    //     this.order.fix_tax_included_price(this);
    //   } else {
    //     console.log("elseeeeeeeeeee", ref_price)
    //     this.set_unit_price(ref_price);
    //     this.order.fix_tax_included_price(this);
    //   }

    //   this.trigger("change", orderline);
    //   return;
    // },
  });
  ...
  models.Order = models.Order.extend({
    updatePricelist: function (newClient) {
      ...
      if (newClient) {
        newClientFiscalPosition = newClient.property_account_position_id
          ? this.pos.fiscal_positions.find(
              (position) =>
                position.id === newClient.property_account_position_id[0]
            )
          : defaultFiscalPosition;
        newClientPricelist =
          this.pos.pricelists.find(
            (pricelist) =>
              pricelist.id === newClient.property_product_pricelist[0]
      ...
      if (
        this.selected_orderline.uom_id != this.selected_orderline.product.uom_id
      ) {
