...
class InsGeneralLedger(models.TransientModel):
    ...
    def action_xlsx(self):
        ...
        if filter.get('include_details', False):
            ...
            sheet.write_string(row_pos, 3, _('Ref'), format_header)
            sheet.write_string(row_pos, 4, _('Move'), format_header)
            sheet.write_string(row_pos, 5, _('Entry Label'), format_header)
            sheet.write_string(row_pos, 6, _('Debit'), format_header)
            sheet.write_string(row_pos, 7, _('Credit'), format_header)
            sheet.write_string(row_pos, 8, _('Balance'), format_header)
        else:
        ...
        if account_lines:
            for line in account_lines:
                row_pos += 1
                sheet.merge_range(row_pos, 0, row_pos, 5, '            ' + account_lines[line].get('code') + ' - ' + account_lines[line].get('name'), line_header_left)
                sheet.write(row_pos, 6, float(account_lines[line].get('debit')), line_header)
                sheet.write(row_pos, 7, float(account_lines[line].get('credit')), line_header)
                sheet.write(row_pos, 8, float(account_lines[line].get('balance')), line_header)
                ...
                if filter.get('include_details', False):
                    ...
                    for sub_line in sub_lines:
                        if sub_line.get('move_name') == 'Initial Balance':
                            row_pos += 1
                            sheet.write(row_pos, 5, sub_line.get('move_name'), line_header_light_initial_bold)
                            ...
                            sheet.write(row_pos, 8, float(sub_line.get('balance')), line_header_light_initial)
                            ...
                            sheet.write(row_pos, 4, sub_line.get('move_name'), line_header_light)
                            sheet.write(row_pos, 5, sub_line.get('lname') or '', line_header_light)
                            ...
                            sheet.write(row_pos, 8, float(sub_line.get('balance')),line_header_light)
                        else: # Ending Balance
                            row_pos += 1
                            sheet.write(row_pos, 5, sub_line.get('move_name'), line_header_light_ending_bold)
                            sheet.write(row_pos, 6, float(account_lines[line].get('debit')), line_header_light_ending)
                            sheet.write(row_pos, 7, float(account_lines[line].get('credit')), line_header_light_ending)
                            sheet.write(row_pos, 8, float(account_lines[line].get('balance')), line_header_light_ending)
